...
def retire_triangles_plats(faces):
    taille = len(faces)
    for i in range(taille):
        if faces[taille-i-1][0] == faces[taille-i-1][1] or faces[taille-i-1][1] == faces[taille-i-1][2] or faces[taille-i-1][2] == faces[taille-i-1][0]:
            #print(f)
            del faces[taille-i-1]
    return faces



# Charger le fichier OBJ
vertices, faces = lire_obj('testobj.obj')
#print(1)

# Appliquer la fusion des points proches avec un seuil de distance
seuil_distance = 40*0.009  # Choisir un seuil approprié
...
encorenewfaces = retire_triangles_plats(new_faces)

sauvegarder_obj('triangles5.obj', new_vertices, encorenewfaces)