...
# EXTRACTION
# =============================================================================================================
def curves_to_closed_loops(points, lines):
    """
    Regroupe les segments par profondeur et reconstruit
    les boucles fermées avec Shapely.
    
    Parameters
    ----------
    points : list of [x, y, depth]
    lines  : list of [i0, i1, depth]
    
    Returns
    -------
    dict : { depth : [LineString fermées] }
    """
    from collections import defaultdict

    segments_by_depth = defaultdict(list)

    # Construction des LineString
    for i, j, d in lines:
        p0 = points[i]
        p1 = points[j]
    ...
    for depth, segments in segments_by_depth.items():
        ...
        loops = [g for g in geoms if g.is_ring]
...
def extraction(depth_values, scale, pas_m, depth_map, height, make_curves=True):
    ...
    if make_curves:
        closed_loops = curves_to_closed_loops(points_list, lines_list)
        return points_list, lines_list, closed_loops

    return points_list, None, None
